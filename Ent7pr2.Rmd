...
# Escalado
preProc_reg <- preProcess(X_train_reg, method = c("center", "scale"))
X_train_reg_scaled <- predict(preProc_reg, X_train_reg)
X_test_reg_scaled <- predict(preProc_reg, X_test_reg)

# Configuración para regresión
ctrl_reg <- trainControl(method = "cv", number = 5)

# Modelo SVR con tuneado
set.seed(42)
...
# Evaluación
y_pred_reg <- predict(svr_model, X_test_reg_scaled)
reg_metrics <- postResample(y_pred_reg, y_reg[-trainIndex])

cat("\nMétricas de regresión:\n")
print(reg_metrics)

# Visualización de resultados real vs predicho
results_df <- data.frame(
  Real = y_reg[-trainIndex],
  Predicho = y_pred_reg
)
...
ggplot(results_df, aes(x = Real, y = Predicho)) +
  ...
  labs(title = "Resultados reales vs predichos - SVR",
       x = "Precio real",
       y = "Precio predicho") +
 theme_minimal()
```
